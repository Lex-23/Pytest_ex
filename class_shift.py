    ...
    def __cmp__(self, other):
        if (self.date_to < other.date_from and
            ...
            if (set(self.week_days) & set(other.week_days)) == set() and \
                    (self.time_from < self.time_to and
                     other.time_from < other.time_to):
                return False
            else:
                if (self.time_from < self.time_to <=
                        other.time_from < other.time_to or
                        other.time_from < other.time_to <=
                        self.time_from < self.time_to or
                        other.time_to <= self.time_from <
                        other.time_from <= self.time_to or
                        self.time_to <= other.time_from <
                        self.time_from <= other.time_to or
                        other.time_from >= self.time_to <
                        other.time_to < self.time_from or
                        self.time_from >= other.time_to <
                        self.time_to < other.time_from):
                    return False
                else:
                    return True
